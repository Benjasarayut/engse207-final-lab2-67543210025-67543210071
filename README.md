...
- นาย ชนสรณ์ บุตรถา รหัส 67543210025-2
- นาย เบญจศรายุทธ  น้อยอุบล รหัส 67543210071-6

## สถานะโปรเจกต์ปัจจุบัน

### สิ่งที่ได้ดำเนินการแล้ว
- อัปเดต `README.md` ให้เป็นเอกสารครบถ้วนสำหรับ **Set 2** รวมถึงภาพรวม, Phase 1-6, Test Cases, และวิธีส่งงาน
- จัดโครงสร้าง Repository ตามที่กำหนด:
  - `auth-service/`: พร้อม Dockerfile, init.sql, package.json, src/ (routes/auth.js, middleware/jwtUtils.js)
  - `task-service/`: พร้อม Dockerfile, init.sql, package.json, src/ (routes/tasks.js, middleware/authMiddleware.js)
  - `user-service/`: พร้อม Dockerfile, init.sql, package.json, src/ (routes/users.js, middleware/authMiddleware.js)
  - `docker-compose.yml`: สำหรับทดสอบ Local (3 services + 3 databases)
  - `screenshots/`: โฟลเดอร์สำหรับเก็บภาพหน้าจอจาก Railway
  - `INDIVIDUAL_REPORT_67543210025.md` และ `INDIVIDUAL_REPORT_67543210071.md`: รายงานรายบุคคล
  - `TEAM_SPLIT.md`: แบ่งงานในทีม
- Git Repository: `engse207-final-lab2-67543210025-67543210071` (branch `blackend`, merged `frontend`)

### สิ่งที่ยังต้องทำ
- **Phase 1**: ปรับ Architecture ให้ทำงานบน Local (เพิ่ม Register API, แยก databases, ทดสอบด้วย docker-compose)
- **Phase 2-4**: Deploy 3 services บน Railway
- **Phase 5**: เลือก Gateway Strategy และทดสอบ End-to-End
- **Phase 6**: จัดทำ Screenshots, อัปเดต TEAM_SPLIT/INDIVIDUAL_REPORT, และ Push Repo

### URL ของ Services บน Railway
- Auth Service: [ยังไม่มี - จะอัปเดตหลัง Deploy]
- Task Service: [ยังไม่มี - จะอัปเดตหลัง Deploy]
- User Service: [ยังไม่มี - จะอัปเดตหลัง Deploy]

### Gateway Strategy ที่เลือก
- **Option A**: Frontend เรียก URL ของแต่ละ service โดยตรง (แนะนำสำหรับการสอบเพื่อลดความซับซ้อน)
...
- ผู้ดูแลระบบ (`admin`) สามารถดูรายชื่อผู้ใช้ทั้งหมดได้
...
1. สร้าง Repository ชื่อ `engse207-final-lab2-[student1]-[student2]`
...
- สมาชิกแต่ละคนต้องสามารถอธิบายส่วนที่ตนรับชอบได้
