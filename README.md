...
เพิ่ม User Service สำหรับจัดการข้อมูลโปรไฟล์ผู้ใช้
แยกฐานข้อมูลออกเป็น 3 ชุด
auth-db
task-db
user-db
ใช้ JWT_SECRET ค่าเดียวกันทุก service
Deploy ขึ้น Railway และทดสอบระบบแบบ end-to-end บน Cloud
ความสามารถที่ระบบควรรองรับ
...
services:
  ...
  user-service:
    ...
    depends_on:
      - user-db
3.9 สิ่งที่ต้องตรวจสอบบน Local ก่อนขึ้น Cloud
POST /api/auth/register สมัครสมาชิกใหม่ได้
POST /api/auth/login ได้ JWT
GET /api/auth/me ได้ข้อมูลผู้ใช้
GET /api/users/me ได้ profile
PUT /api/users/me อัปเดต profile ได้
POST /api/tasks ทำงานได้
GET /api/tasks ทำงานได้
ทุก service เชื่อมต่อ database ของตนเองได้
...
4. Phase 2: Deploy Auth Service บน Railway
ไปที่ Railway → New Project → Deploy from GitHub
เลือก repository และกำหนด Root Directory = auth-service
เพิ่ม PostgreSQL Plugin และตั้งชื่อ auth-db
ตั้งค่า Environment Variables ดังนี้
DATABASE_URL=${{auth-db.DATABASE_URL}}
JWT_SECRET=your-shared-jwt-secret-here
JWT_EXPIRES_IN=1h
PORT=3001
NODE_ENV=production
Deploy และตรวจสอบ Deploy Logs
ทดสอบ endpoint อย่างน้อย:
POST /api/auth/register
POST /api/auth/login
GET /api/auth/me
GET /api/auth/health
บันทึก Auth Service URL เพื่อใช้ทดสอบ End-to-End
...
5. Phase 3: Deploy Task Service บน Railway
เพิ่ม New Service ใน Project เดียวกัน และเลือก Deploy from GitHub
กำหนด Root Directory = task-service
เพิ่ม PostgreSQL Plugin และตั้งชื่อ task-db
ตั้งค่า Environment Variables ดังนี้
...
6. Phase 4: Deploy User Service บน Railway
เพิ่ม New Service และกำหนด Root Directory = user-service
เพิ่ม PostgreSQL Plugin และตั้งชื่อ user-db
ตั้งค่า Environment Variables ดังนี้
...
9. วิธีการส่งงาน
Git Repository
สร้าง Repository ชื่อ engse207-final-lab2-[รหัส1]-[รหัส2]
โครงสร้าง repository ให้ต่อเนื่องจาก Set 1 โดยเพิ่ม user-service/ และแยก init.sql ตามแต่ละ service
อัปเดต README.md ให้มี URL จริงของทุก service บน Railway
ส่ง URL Repository ผ่านระบบของมหาวิทยาลัย
ไฟล์บังคับที่ต้องมี
README.md
TEAM_SPLIT.md
...
ไม่ต้องทำ frontend นะ ทำหลังบ้านก็พอ